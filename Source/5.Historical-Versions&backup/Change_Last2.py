        ...
        try:
            def weight(u, v, d):
                ...
                if d['capacity'] >= bandwidth_need:
                    fragmentation_penalty = alpha * (1 / (d['capacity'] - bandwidth_need + 1))
                    return (1 / d['capacity']) + fragmentation_penalty
                else:
                    return float('inf')  # 返回无限大，表示不可选
    ...
    for von_number, von in enumerate(vons, start=1):
        ...
        for (vn_source, vn_target, data) in von.edges(data=True):
            ...
            if best_path:
                ...
                # 备用资源失败，将该请求放入等待队列，并记录等待时间

                print(f"Mapping failed for VON {von_number}, adding to waiting queue...")
                waiting_queue.append((von_number, vn_source, vn_target, data, current_time))
