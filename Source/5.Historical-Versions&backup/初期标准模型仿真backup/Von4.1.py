...
def recover_resources(aux_network, von_mappings):
    ...
    while end_times and end_times[0] <= current_time:
        end_time = end_times.pop(0)  # 移除最早结束的时间
...
def recover_resources_at_time(aux_network, von_mappings, end_time):
    for von_number, mapping in list(von_mappings.items()):
        nodes_to_remove = [vn_id for vn_id, pn_id in mapping['node_mappings'].items() if mapping['node_mappings'][vn_id]['end_time'] == end_time]
        for vn_id in nodes_to_remove:
            pn_id = mapping['node_mappings'][vn_id]
            # 恢复节点资源
            aux_network.nodes[pn_id]['computing_resource'] += aux_network.nodes[pn_id].pop('von_' + str(von_number) + '_resource', 0)
            # 删除节点映射
...
for attempt in range(total_attempts):
    ...
    aux_network = physical_network.copy()
    # 增加时间类的参数
    lambda_range = np.arange(4.5, 72.5, 0.5)
    lambda_value = 1 / lambda_range[0]  # 选择一个到达率值进行测试
    mu = 1  # 平均服务率
...
# 假设模拟进行到了某个时间点，我们需要检查并恢复资源
current_time = 1000  # 假设当前时间点为100
von_mappings = {}
recover_resources(aux_network, von_mappings, current_time)
