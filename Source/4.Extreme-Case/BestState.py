...
# lpm的节点放置方法 - 从小到大
def lpm_algorithm_optimized(physical_network, vons):
    aux_network = physical_network.copy()
    von_mappings = {}
    ...
    for von_number, von in enumerate(vons, start=1):
        ...
        # 节点映射：从小到大排列虚拟节点，优先放置资源消耗最小的节点
        for vn_id, vn_data in sorted(von.nodes(data=True), key=lambda x: x[1]['computing_resource']):
        ...
        # 链路映射：从小到大排列带宽需求，优先映射带宽需求最小的虚拟链路
        for (vn_source, vn_target, data) in sorted(von.edges(data=True), key=lambda x: x[2]['bandwidth']):
...
for attempt in range(total_attempts):
    ...
    # 选择映射算法
    success, post_network = lpm_algorithm_optimized(aux_network, vons)
...
if successful_attempts > 0:
    ...
    for u, v, data in physical_network.edges(data=True):
        initial_capacity = data['capacity']  # 初始容量
        post_capacity = post_network[u][v]['capacity']  # 剩余容量

        # 计算这条边实际使用的容量
        used_capacity = initial_capacity - post_capacity
        total_capacity_used += used_capacity

    # 计算平均每次尝试的使用容量
    average_capacity_used_per_attempt = total_capacity_used / total_attempts
