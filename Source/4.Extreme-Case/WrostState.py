...
def random_node_and_link_placement(physical_network, vons):
    ...
    for von_number, von in enumerate(vons, start=1):
        ...
        for vn_id, vn_data in von.nodes(data=True):
            available_nodes = list(aux_network.nodes(data=True))  # 获取所有物理节点
            random.shuffle(available_nodes)  # 随机打乱物理节点列表
        ...
        for (vn_source, vn_target, data) in von.edges(data=True):
            ...
            # 获取物理网络中可能的邻居节点
            physical_paths = list(nx.all_simple_paths(aux_network, source=pn_source, target=pn_target))
            random.shuffle(physical_paths)  # 随机打乱路径顺序
...
def npm_algorithm_max_blocking(physical_network, vons):
    aux_network = physical_network.copy()
    von_mappings = {}
    ...
    for von_number, von in enumerate(vons, start=1):
        ...
        for (vn_source, vn_target, data) in von.edges(data=True):
            ...
            try:
                ...
                return False, aux_network  # 没有找到路径，映射失败
...
def lpm_algorithm_priority_misallocation(physical_network, vons):
    aux_network = physical_network.copy()
    von_mappings = {}
    ...
    for von_number, von in enumerate(vons, start=1):
        ...
        # 链路映射：优先映射带宽需求大的链路
        for (vn_source, vn_target, data) in sorted(von.edges(data=True), key=lambda x: x[2]['bandwidth'], reverse=True):
...
for attempt in range(total_attempts):
    ...
    # 选择映射算法
    success, post_network = random_node_and_link_placement(aux_network, vons)
