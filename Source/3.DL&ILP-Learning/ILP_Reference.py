...
def ilp_algorithm(physical_network, vons ,waiting_queue):
    ...
    return mapping_success, aux_network
def random_mapping_algorithm(physical_network, vons, waiting_queue):
    aux_network = physical_network.copy()
    von_mappings = {}
    mapping_success = True
    ...
    for von_number, von in enumerate(vons, start=1):
        ...
        for vn_id, vn_data in von.nodes(data=True):
            # 随机选择一个物理节点
            available_nodes = list(aux_network.nodes())
            random.shuffle(available_nodes)
            for pn_id in available_nodes:
                if vn_data['computing_resource'] <= aux_network.nodes[pn_id]['computing_resource']:
                    aux_network.nodes[pn_id]['computing_resource'] -= vn_data['computing_resource']
                    von_mappings[von_number]['node_mappings'][vn_id] = pn_id
                    break
            else:
        ...
        for (vn_source, vn_target, data) in von.edges(data=True):
            if not mapping_success:
                break
            # 随机选择路径
            source = von_mappings[von_number]['node_mappings'][vn_source]
            target = von_mappings[von_number]['node_mappings'][vn_target]
            # 查找所有可能的路径
            paths = nx.all_shortest_paths(aux_network, source, target)
            if not paths:
                mapping_success = False
                break
            # 随机选择一条路径
            ...
            for i in range(len(path) - 1):
                aux_network[path[i]][path[i+1]]['capacity'] -= adjusted_bandwidth + random.randint(1, 10)
            von_mappings[von_number]['link_mappings'][(vn_source, vn_target)] = path

    return mapping_success, aux_network


def badlpm_algorithm(physical_network, vons):
    aux_network = physical_network.copy()
    von_mappings = {}
...
for attempt in range(total_attempts):
    ...
    success, post_network = random_mapping_algorithm(aux_network, vons, waiting_queue)
